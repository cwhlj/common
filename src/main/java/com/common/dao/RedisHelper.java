...
/**
 *
 * @Description: redis集群操作工作
 * 				   注意：get函数可以使用class作为参数
 * 					  当获取值为List、Set等，需要注明  实体类型，否则将转化为JSONOBJECT
 * 					  当获取值包含泛型时，需要试题类型，否则将转化为JSONOBJECT
 *
 * 					 type定义
 * 					 Type type = new TypeReference<TestEntity<SampleEntity>>() {}.getType();
 */
